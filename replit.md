...
-   **Sub-Hub Warehouse Roles and Dispatch Workflow**: Separates planning/approval from physical dispatch operations with strict scope-limited access controls. Logistics Officers and Managers plan and approve fulfilments with visibility across all hubs, while Warehouse Supervisors and Officers at Sub-Hubs have restricted access limited to their assigned location:
    ...
    -   **Request Fulfilment Change Workflow**: Enables Warehouse Supervisors and Officers to request adjustments to approved needs list fulfilments when encountering issues during dispatch preparation. Features include:
        -   **Hub-Scoped Security**: Warehouse users can only request changes for needs lists where their specific Sub-Hub has actual fulfilment allocations. Cross-hub spoofing is prevented via server-side validation that filters by both needs_list_id AND source_hub_id
        -   **Request Submission**: "Request Fulfilment Change" button and modal form on approved needs lists shows all items (read-only) with required comments field explaining the issue (e.g., "Wrong hub assigned", "Quantity needs adjustment", "Stock unavailable")
        -   **Logistics Review**: Logistics Officers and Managers receive in-app notifications and can view all pending change requests on the needs list detail page. They can respond with three actions: Approve & Update (approve the request and manually update fulfilment), Reject (deny the request with explanation), or Need Clarification (request more information)
        -   **Feedback Loop**: Warehouse users receive in-app notifications when their requests are processed, with status badges (Pending Review/Approved & Updated/Rejected/Clarification Needed) and Logistics team responses displayed on the needs list detail page
        -   **Data Model**: FulfilmentChangeRequest table tracks all requests with relationships to NeedsList, requesting_hub, requested_by, and reviewed_by, ensuring full audit trail and accountability
